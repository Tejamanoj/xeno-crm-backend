...
if (existing.count === 0) {
  const insert = db.prepare(`
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?)
  `);
  customers.forEach(c => insert.run(uuid(), c.name, c.email, c.phone, c.segment, c.total_orders, c.total_spent));
  console.log("✅ Seeded customers");
}

const segs = db.prepare("SELECT COUNT(*) as count FROM segments").get();
if (segs.count === 0) {
  const ins = db.prepare(`INSERT INTO segments (id, name, description, channel) VALUES (?, ?, ?, ?)`);
